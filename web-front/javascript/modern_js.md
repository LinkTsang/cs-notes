...
[队列（queue）](https://en.wikipedia.org/wiki/Queue_(abstract_data_type))
...
最近，`globalThis` 被作为全局对象的标准名称加入到了 JavaScript 中，所有环境都应该支持该名称。在有些浏览器中，即 non-Chromium Edge，尚不支持 `globalThis`，但可以很容易地对其进行 polyfilled。
...
- 函数就是值
- 每个值都有一种类型
- 函数就是对象，即可被调用的“行为对象（action object）”
...
```javascript
let sayHi = function func(who) {  // 内部名，func 只在内部可见
  if (who) {
    alert(`Hello, ${who}`);
  } else {
    func("Guest"); // 使用 func 再次调用函数自身，防止外面修改sayHi
  }
};
```
...
场景：从服务器获取代码或者动态地从模板编译函数时

### [闭包](https://zh.javascript.info/new-function#bi-bao)

使用 `new Function` 该函数的 `[[Environment]]` 并不指向当前的词法环境，而是指向全局环境。



## [调度：setTimeout 和 setInterval](https://zh.javascript.info/settimeout-setinterval)[象属性配置](https://zh.javascript.info/object-properties)
